"""
app_tester.py — Salt Dispenser with Gemini-powered Voice Assistant
===================================================================
Hybrid voice system:
  - "dispense salt" / hardware commands → handled directly (no AI needed)
  - Everything else → sent to Gemini with live ESP context → spoken by ElevenLabs

Install dependencies:
  pip install requests speechrecognition elevenlabs pygame google-generativeai

Build exe:
  pip install pyinstaller
  pyinstaller --onefile --windowed --name "Salt Dispenser" app_tester.py
...
ELEVENLABS_KEY   = "sk_c9ae5ff6b89f09bfc47e876400e3a803f6a5a99b94e4899b"    # ← paste ElevenLabs key here
ELEVENLABS_VOICE = "21m00Tcm4TlvDq8ikWAM"       # Rachel — free tier voice

GEMINI_KEY       = "AIzaSyCV_QQYuS2tf8WcEf-L9jApVURkAQolBjU"         # ← paste Gemini key here
...
MIC_LISTEN  = "#e84040"
MIC_THINK   = "#b45309"
MIC_GEMINI  = "#7c3aed"   # purple — shows when Gemini is thinking
...
# ══════════════════════════════════════════════════════════════════════════════
#  GEMINI ENGINE
#  Replaces the old CommandMatcher response logic.
#  Receives spoken text + live ESP data → returns a short spoken response.
# ══════════════════════════════════════════════════════════════════════════════
class GeminiEngine:
    def __init__(self, api_key: str):
        if GEMINI_OK and api_key != "YOUR_GEMINI_API_KEY":
            ...
            self.ready = True
            print("[GEMINI] Connected — Gemini 2.0 Flash ready")
        else:
            self.client = None
            self.ready = False
    ...
    def ask(self, user_text: str, esp_data: dict) -> str:
        """
        Send user's spoken question to Gemini with live ESP context.
        Returns a short spoken response string.
        """
        if not self.ready:
            return self._fallback(user_text, esp_data)

        # Build a context summary from live ESP data
        if esp_data:
        ...
        try:
            result = self.client.models.generate_content(model = 'gemini-2.5-flash', contents=prompt)
    ...
    def _fallback(self, text: str, esp_data: dict) -> str:
        """Basic fallback if Gemini is not configured."""
        text = text.lower()
        if not esp_data:
        ...
        if any(w in text for w in ["dispensing", "salt", "walkway", "status"]):
            return "Salt is being dispensed right now." if dispensing else "Your walkway is being monitored and no salt is needed right now."
...
#  Only thing still hardcoded — hardware control should never go through AI.
# ══════════════════════════════════════════════════════════════════════════════
DISPENSE_KEYWORDS = [
...
def is_dispense_command(text: str) -> bool:
    text = text.lower()
    return any(kw in text for kw in DISPENSE_KEYWORDS)


# ══════════════════════════════════════════════════════════════════════════════
#  VOICE ENGINE  (unchanged from app.py)
# ══════════════════════════════════════════════════════════════════════════════
class VoiceEngine:
...
class SaltDispenserApp(tk.Tk):
    ...
    def _build(self):
        ...
        # Shows what the user said + which engine handled it
    ...
    def _handle_command(self, text: str):
        """
        Hybrid routing:
          1. Empty / no speech  → apologise
          2. Dispense keywords  → trigger hardware directly, no AI
          3. Everything else    → send to Gemini with ESP context
        """
        ...
        # ── Path 1: hardware dispense command ──
        ...
        # ── Path 2: everything else goes to Gemini ──
